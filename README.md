...
- `INC <arg>` -- инкрементировать операнд (+1). *Регистровая операция*
- `DEC <arg>` -- декрементировать (-1) операнд. *Регистровая операция*
...
Зарезервированные переменные:
  - Возможно обратиться только напрямую через адрес памяти (#STDIN)
  - Имеют фиксированный адрес памяти
